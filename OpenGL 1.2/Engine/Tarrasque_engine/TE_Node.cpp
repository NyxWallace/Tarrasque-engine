...
{
	for (vector<TE_Node*>::iterator iter = m_children->begin(); iter != m_children->end(); ++iter) {
		if (*iter == node){
...
*/
LIB_API TE_Node* TE_Node::find_node(string name){
	for (vector<TE_Node*>::iterator iter = m_children->begin(); iter != m_children->end(); ++iter) {
		TE_Node* tmp = nullptr;
...
*/
LIB_API vector<TE_Node*>* TE_Node::get_children(){
	return m_children;
}
...
*/
void LIB_API TE_Node::set_name(string name){
	m_name = name;
}
...
*/
LIB_API string TE_Node::get_name(){
	return m_name;
}
...
*/
LIB_API TE_Node::TE_Node(string name) : TE_Object(), m_name(name), m_children(new vector<TE_Node*>()), m_parent(nullptr), m_position(TE_MAT4()){
