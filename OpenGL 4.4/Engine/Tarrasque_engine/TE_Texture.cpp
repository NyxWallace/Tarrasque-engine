...
LIB_API TE_Texture::TE_Texture(std::string textureName, int anisotropy) : TE_Object(){
	...
	gluBuild2DMipmaps(GL_TEXTURE_2D,GL_COMPRESSED_RGB, FreeImage_GetWidth(bitmap), FreeImage_GetHeight(bitmap), GL_BGRA_EXT, GL_UNSIGNED_BYTE, (void *)FreeImage_GetBits(bitmap));
}

LIB_API TE_Texture::TE_Texture(unsigned int tex) : TE_Object(){
	m_type = Object_type::TEXTURE;
	m_tex_id = tex;
}
...
LIB_API void TE_Texture::create(int x, int y){
